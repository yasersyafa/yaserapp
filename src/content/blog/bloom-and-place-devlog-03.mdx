...
You don't have to look hard for examples in Unity. The classic shape is `SystemA.Instance.DoThing()` called from anywhere. Every static call site is a piece of knowledge wedged into a place that didn't need it. Move the method, rename the manager, change the signature — half the codebase lights up.
...
The whole research thrust here is: coupling is a measurable property, not a taste. Sixty years of structured design says the same thing. Bloom & Place is the lab; the Ce/Ca numbers I posted in devlog #2 are the dial; VContainer + C# events are the controls. If the dial doesn't move, the controls don't work — and that's a result, even if it's not the one I'm hoping for.
