...
In practice, constructor injection is the one I reach for and the one I'd recommend reaching for. It makes dependencies impossible to forget — the class won't compile without them — and impossible to change at runtime, which kills a whole category of "wait, who set this?" bugs.
...
- *"MonoBehaviours can't have constructors."* True, and it's why this matters. Forcing yourself toward DI also forces you toward putting domain logic in plain C# classes that *can* have constructors — which is the architectural improvement the thesis is actually after. MonoBehaviours stay as views and input adapters.
...
- MonoBehaviours are leaves — view + input only. They get the systems they need injected via VContainer.
